...
function drawRoulette(names) {
    ...
    names.forEach((name, index) => {
        ...
        ctx.fillStyle = pastelColors[index % pastelColors.length]; // Utilitza colors pastel
        ctx.fill();
        ctx.stroke();
        ctx.save();
        ctx.translate(150, 150);
        ctx.rotate(index * segmentAngle + segmentAngle / 2);
        ctx.fillStyle = "#000"; // Negreta per al text
        ctx.font = "bold 14px Arial"; // Nom en negreta
